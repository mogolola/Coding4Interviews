...
function DFS(matrix:string[][], row:number, col:number, path:string, visited:any[]):boolean{
    let pos = [row, col].join(",")
    if(row<0 || row>matrix.length || col<0 || col>matrix[0].length || visited.indexOf(pos)>=0)  return false;
...
function hasPath(matrix:string, rows:number, cols:number, path:string)
{
    // write code here
    let mat = [];
    ...
    for(let i=0; i<rows;i++){
        for(let j=0; j<cols; j++){
            let visited:[] = [];
            if (DFS(mat, i, j, path, visited))    return true
        }
    }
